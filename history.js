document.getElementById('home_button').addEventListener('click', function() {
    window.location.href = 'login.html';
  });
  
  document.getElementById('logout_button').addEventListener('click', function() {
    window.location.href = 'login.html';
  });


function searching()
{
    var books = [];
    var room = [];
    var names = [];
    var time1 = [];
    var time2 = [];
    var final_dates = [];
    var bookings = [];

    const tbody = document.getElementById('his_table');
    var date1 = document.getElementById("date1").value;
    var date2 = document.getElementById("date2").value;

    date1 = date1.trim().split("-");
    date2 = date2.trim().split("-");
    console.log("looking for : "+date1);
    console.log("looking for : "+date2);

    keys = Object.keys(localStorage);

    if(keys !== null)
    {
        for(var i = 0; i < keys.length; i++)
        {
            if(keys[i].startsWith("book"))
            {
                bookings.push(keys[i]);
            }
        }

        for(var i = 0; i < bookings.length; i++)
        {
            var data = localStorage.getItem(bookings[i]);
            data = data.trim().split(">");


            if(isDateInRange(data[2], date1, date2))
            {
                room.push(data[0]);
                names.push(data[1]);
                final_dates.push(data[2]);
                time1.push(data[3]);
                time2.push(data[4]);
                books.push("book" + i);
            }
            else
            {
                clearTable();
                alert("No bookings found !");
                console.log("nothing inside")
            }
        }

        clearTable();
        insert_rows(names, room, final_dates, time1, time2, books, tbody);
    }

    else
    {
        clearTable();
        alert("No bookings found !");
        console.log("nothing inside")
    }

}



function clearTable() {
    var tableBody = document.getElementById('his_table');
    

    while (tableBody.rows.length > 1) {
        tableBody.deleteRow(1);
    }
}


function isDateInRange(targetDate, startDate, endDate) {

    targetDate = new Date(targetDate);
    startDate = new Date(startDate);
    endDate = new Date(endDate);

    return targetDate >= startDate && targetDate <= endDate;
}


function insert_rows(names, room, final_dates, time1, time2, books, tbody)
{
    for (var i = 0; i < books.length; i++)
    {
        var newRow = tbody.insertRow(tbody.rows.length);
        var cell1 = newRow.insertCell(0);
        var cell2 = newRow.insertCell(1);
        var cell3 = newRow.insertCell(2);
        var cell4 = newRow.insertCell(3);
        var cell5 = newRow.insertCell(4);

        cell1.textContent = names[i];
        cell2.textContent = room[i];
        cell3.textContent = final_dates[i];
        cell4.textContent = time1[i];
        cell5.textContent = time2[i];
    }
}